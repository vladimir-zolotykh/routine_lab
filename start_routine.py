...
class RoutineEditor(tk.Toplevel):
    ...
    def add_exercise(self, ex_box: tk.Frame) -> None:
        ...
        ex_frame.grid(sticky=tk.EW)
        ttk.Combobox(ex_frame).grid(column=0, sticky=tk.W)
        weight = tk.Entry(ex_frame)
        wo_set.append(weight)
        weight.grid(column=1, sticky=tk.W)
        reps = tk.Entry(ex_frame)
        wo_set.append(reps)
        self.wo_exercises[ex_frame] = wo_set
        reps.grid(column=2, sticky=tk.W)
        del_btn = tk.Button(
            ex_frame, text="Delete", command=lambda: self.remove_exerecise(ex_frame)
        )
        del_btn.grid(column=3, sticky=tk.W)
